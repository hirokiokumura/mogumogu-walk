...
Server Component は、ブラウザではなく Next.js 側で描画されるコンポーネントです。通常のサーバーあり構成ではリクエスト時にサーバーで描画されることがありますが、このプロジェクトは `next.config.ts` で `output: "export"` を使う静的エクスポート構成です。

そのため、このプロジェクトでは Server Component の描画は `npm run build` のビルド時に行われます。GitHub Pages に配置されたあと、本番環境で Next.js のサーバー処理が動くわけではありません。

Server Component では、次のようなブラウザ専用 API を直接使えません。
...
また、次のような React のインタラクティブな機能も Client Component で使います。
...
React ではコンポーネントが再描画されるたびに、コンポーネント内で定義した関数も新しく作られます。通常は問題ありませんが、次のような場面では「同じ関数として扱われること」が重要になります。

- `React.memo` した子コンポーネントに関数を props として渡すとき
- `useEffect` の依存配列に関数を入れるとき
- カスタム Hook から安定した関数を返したいとき
...
この例では、`stop` の中で使っている外側の値は `setIsOn` だけです。`setIsOn` は React が返す state 更新関数で、参照が安定しているため依存配列に入れる必要がありません。そのため依存配列は空配列 `[]` で問題ありません。

一方で、関数内で props や state を参照する場合は、それらを依存配列に入れます。依存配列が間違うと、古い値を参照するバグにつながることがあります。
...
第1引数の `subscribeToSessions` は、保存データが変わったときに React へ通知するための購読関数です。第2引数の `getSessionsSnapshot` は、ブラウザ上で現在の localStorage から値を読む関数です。

第3引数の `getSessionsServerSnapshot` は、サーバー側またはビルド時に使う代替の読み取り関数です。このプロジェクトは静的エクスポートなので、`npm run build` の時点では `localStorage` が存在しません。そこで空配列などの安全な初期値を返すことで、ビルド時のエラーや hydration の不一致を避けています。
...
実務では、生成物や一時的な作業ディレクトリを lint 対象から除外する設定も重要です。プロジェクト固有の作業ファイルが混ざる場合は、ESLint や Biome の ignore 設定で対象範囲を明確にします。
